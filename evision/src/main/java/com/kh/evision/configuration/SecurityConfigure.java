...
public class SecurityConfigure {
	...
	// application.properties에서 설정 가능하도록
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
		return httpSecurity.formLogin(AbstractHttpConfigurer::disable)
				   ...
				   .authorizeHttpRequests(requests -> {
					   ...
					   */
					   requests.anyRequest().permitAll();  // 임시로 모두 허용!
				   })
				   .sessionManagement(manager ->
						   				manager.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
				   // .addFilterBefore(jwtFilter, UsernamePasswordAuthenticationFilter.class) 임시 주석 처리.
				   .build();  
	}
	...
	public CorsConfigurationSource corsConfigurationSource() {
	    ...
	    configuration.setExposedHeaders(Arrays.asList("Authorization"));  // 응답 헤더 노출
